...
for sequence in "${SEQUENCES[@]}" do
    for method in "${METHODS[@]}" do
        ...
        tag="Siren${method}"
        python launch.py --config ./configs/dysdf/$method.yaml dataset.scene=$sequence --exp_dir ../exp_owlii_benchmark --train model.hidden_features=$method tag=$tag
        # run training of vanilla baseline + ResFields
        python launch.py --config ./configs/dysdf/$method.yaml dataset.scene=$sequence --exp_dir ../exp_owlii_benchmark --train model.sdf_net.resfield_layers=[1,2,3,4,5,6,7] model.hidden_features=$method tag=ResFields1234567$tag
    done
