...
def get_ext(filename: str, match_first: bool = False):
    if match_first:
...
def mkdir(dst: str, exist_ok: bool = True, **kwargs) -> None:
    os.makedirs(dst, exist_ok=exist_ok, **kwargs)

def _dispatch(registry: Dict[str, Callable], name: Literal["load", "dump"]) -> Callable:
    def _dispatched(filename: str, *args, **kwargs):
        ext = get_ext(filename, match_first=False)
        func = registry[ext]
        if (
            name == "dump"
...
@_register()
def load_txt(
    filename: str, *, strip: bool = True, **kwargs
) -> List[str]:
...
@_register()
def dump_txt(filename: str, obj: List[Any], **_) -> None:
...
@_register()
def load_pkl(filename: str, **kwargs) -> Dict:
...
@_register()
def dump_pkl(filename: str, obj: Dict, **kwargs) -> None:
...
@_register()
def load_npy(
    filename: str, *, allow_pickle: bool = True, **kwargs
) -> Union[np.ndarray, Dict[str, np.ndarray]]:
    return np.load(filename, allow_pickle=allow_pickle, **kwargs)


@_register()
def dump_npy(filename: str, obj: np.ndarray, **kwargs) -> None:
    np.save(filename, obj, **kwargs)


@_register()
def load_npz(filename: str, **kwargs) -> np.lib.npyio.NpzFile:
    return np.load(filename, **kwargs)


@_register()
def dump_npz(filename: str, **kwargs) -> None:
...
def load_img(
    filename: str, *, use_rgb: bool = True, **kwargs
) -> np.ndarray:
...
def load_vid_metadata(filename: str) -> np.ndarray:
