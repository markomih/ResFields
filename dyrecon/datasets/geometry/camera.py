...
def project(
    ...
    *,
    ...
    if isinstance(points, np.ndarray):
        ...
        assert all([isinstance(x, torch.Tensor) for x in tensors_to_check])
        # np_or_jnp = jnp
        np_or_torch = torch
    ...
    normalized_pixels = np_or_torch.where(
        local_points[..., -1:] != 0,
    ...
    if tangential_distortions is not None:
        ...
        tangential_deltas = 2 * tangential_distortions * np_or_torch.prod(
            normalized_pixels,
            axis=-1,
            keepdims=True,
    ...
    pixels = utils.matv(
        ...
        np_or_torch.concatenate(
            [
                normalized_pixels,
                # np_or_jnp.ones_like(normalized_pixels[..., :1]),
                np_or_torch.ones_like(normalized_pixels[..., :1]),
            ],
            axis=-1,
        ),
    )[..., :2]
    ...
    if not return_depth:
        ...
        depths = (
            ...
            else np_or_torch.linalg.norm(local_points, axis=-1, keepdims=True)
        )
...
class Camera(object):
    ...
    # def pixels_to_rays(self, pixels : torch.Tensor) -> struct.Rays:
        viewdirs = self.pixels_to_viewdirs(pixels)
        # viewdirs = torch.from_numpy(viewdirs)
        # TODO(Hang Gao @ 07/20): Use viewdirs as directions can be an issue.
        return struct.Rays(
            origins=np.broadcast_to(self.position, viewdirs.shape),
            # origins=torch.broadcast_to(torch.from_numpy(self.position), viewdirs.shape),
            directions=viewdirs,
        )
    def pixels_to_rays_np(self, pixels: np.ndarray):
        viewdirs = self.pixels_to_viewdirs(pixels)
        return np.concatenate((
            np.broadcast_to(self.position, viewdirs.shape),
            viewdirs
        ), axis=-1)
