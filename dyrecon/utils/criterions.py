...
#     def _convolve2d(z, m, f):
#         # z_ = jsp.signal.convolve2d(
#             # z * m, f, mode="valid", precision=jax.lax.Precision.HIGHEST
#         # )
#         z = z.detach().cpu().numpy() if torch.tensor(z) else z
#         m = m.detach().cpu().numpy() if torch.tensor(m) else m
...
#             # m_ != 0
#         # ).astype(z.dtype)
#         return np.where(m_ != 0, z_ * np.ones_like(f).sum() / m_, 0), (
#             m_ != 0
#         ).astype(z.dtype)
#     filt_fn1 = lambda z, m: _convolve2d(z, m, filt[:, None])
#     filt_fn2 = lambda z, m: _convolve2d(z, m, filt[None, :])

#     # Vmap the blurs to the tensor size, and then compose them.
#     # TODO possible errors
#     # filt_fn1 = jax.vmap(filt_fn1, in_axes=(2, None), out_axes=(2, None))
#     # filt_fn1 = lambda z, m: filt_fn1(*filt_fn2(z, m))
#     # TODO possible errors
#     # filt_fn2 = jax.vmap(filt_fn2, in_axes=(2, None), out_axes=(2, None))
#     # filt_fn2 = lambda z, m: filt_fn2(*filt_fn1(z, m))
...
#     sigma00 = torch.maximum(torch.tensor(0.0), sigma00)
#     # sigma11 = jnp.maximum(0.0, sigma11)
#     sigma11 = torch.maximum(torch.tensor(0.0), sigma11)
