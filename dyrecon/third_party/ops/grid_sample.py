...
def grid_sample_2d_bicubic(input, grid, padding_mode='zeros', align_corners=True):
    assert padding_mode in ['zeros', 'border']
    return _GridSample2dForward.apply(input, grid, padding_mode, align_corners, 'bicubic')
...
class _GridSample2dForward(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, grid, padding_mode=0, align_corners=True, mode='bilinear'):
        assert input.ndim == 4
        assert grid.ndim == 4
        ...
        output = torch.nn.functional.grid_sample(input=input, grid=grid, mode=mode,
...
class _GridSample2dBackward(torch.autograd.Function):
    ...
    def forward(ctx, grad_output, input, grid, padding_mode=0, align_corners=True):
        ...
        if _use_pytorch_1_11_api:
            # bicubic: ctx.needs_input_grad=(True, True, True, False, False), bilinear: (True, True, True, False, False)
